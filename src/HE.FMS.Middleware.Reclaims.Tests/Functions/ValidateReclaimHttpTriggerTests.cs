...
public class ValidateReclaimHttpTriggerTests : IAsyncDisposable
{
...
#pragma warning restore CA2213
    ...
    private readonly ValidateReclaimHttpTrigger _function;

    private bool _disposed;

    public ValidateReclaimHttpTriggerTests()
    {
        var cosmosDbSettings = new CosmosDbSettings { DatabaseId = "test", ContainerId = "test" };
        var serviceBusSettings = new ServiceBusSettings { ReclaimsTopic = "reclaims-topic" };
        var allowedEnvironmentSettings = new AllowedEnvironmentSettings("test");

        var logger = Substitute.For<ILogger<ValidateReclaimHttpTrigger>>();
        ...
        serviceBusClient.CreateSender(serviceBusSettings.ReclaimsTopic).Returns(_serviceBusSender);
        ...
        _function = new ValidateReclaimHttpTrigger(
            streamSerializer,
            _traceCosmosClient,
            objectSerializer,
            _clientFactory,
    ...
    ~ValidateReclaimHttpTriggerTests()
    {
        DisposeAsync(false).AsTask().GetAwaiter().GetResult();
    }

    [Fact]
    public async Task Run_ShouldValidateReclaim()
    {
        // Arrange
