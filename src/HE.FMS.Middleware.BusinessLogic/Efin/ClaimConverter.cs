...
public class ClaimConverter : IClaimConverter
{
    ...
    public async Task<CLCLB_Batch> CreateBatch(IEnumerable<ClaimItem> claims, string batchRef)
    {
        ...
        return new CLCLB_Batch()
        {
            ...
            clb_entry_date = _dateTimeProvider.UtcNow.ToString("d-MMM-yy", CultureInfo.InvariantCulture),
        };
    }
    ...
    public async Task<CLI_Invoice> CreateCliInvoice(ClaimPaymentRequest claimPayment)
    {
        ...
        return new CLI_Invoice()
        {
            ...
            cli_description = string.Format(CultureInfo.InvariantCulture, "{0}{1}", claimPayment.Application.SchemaName.TrimEnd(3), milestoneLookup[claimPayment.Claim.Milestone]),
    ...
    public async Task<CLA_InvoiceAnalysis> CreateClaInvoiceAnalysis(ClaimPaymentRequest claimPayment)
    {
        ...
        return new CLA_InvoiceAnalysis()
        {
            ...
            cla_cfacs_ac = partnerTypeLookup[$"Claim_{claimPayment.Application.RevenueIndicator}_{claimPayment.Account.PartnerType.RemoveSpecialCharacters()}"],
            ...
            cla_description = string.Format(CultureInfo.InvariantCulture, "{0}{1}", claimPayment.Application.SchemaName.TrimEnd(3), milestoneLookup[claimPayment.Claim.Milestone]),
